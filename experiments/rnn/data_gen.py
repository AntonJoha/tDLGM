...
class Datagen():
    ...
    def get_true_data(self, seq_len, variance=0, probability=0):
        if variance!=0:
            x,y = self.make_data(self.df, self.device, seq_len)
            self.cache_true[seq_len] = self.true_data(x,y,self.device)
            
            return self.feature_engineering(x,y,self.device,variance,probability)
    ...
    def get_test_data(self, seq_len, variance=0, probability=0):
    ...
    def feature_engineering(self, X,Y, device, variance=0.01, probability=0.1):
        ...
        for x, y in zip(X,Y):
            for i in range(count):
                ...
                new_y.append(y_val)
                if random.random() < probability:
                    y = y + np.random.normal(loc=0,scale=variance)
                if y > 1:
                    y = 1
                if y < 0:
                    y = 0
