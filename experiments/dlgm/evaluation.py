...
def _generate_test(m, m_r, seq_len, variance=0, prob=0,times=10):
    ...
    for i in range(times):
        res = get_yhat(m,m_r, x_test_1, forcing=True, seq_len=1).to(device)
        results.append(res)
...
def violin_plot(t, title="Violin plot"):

    fig, axs = plt.subplots(1)


# plot violin plot
    axs.violinplot(t.cpu(),
                  showmeans=False,
                  showmedians=True)
    axs.set_title(title)
...
def hist_plot(t, title="Histogram"):
    fig, axs = plt.subplots(1)

    axs.hist(t.cpu(), bins=(500))
    axs.set_title(title)

    plt.show()


def generate_test(m, m_r, conf, times=10):
    ...
    for v, p in zip(var, data_prob):
        results, results_diff = _generate_test(m, m_r ,conf["seq_len"], conf["variance"], conf["data_prob"], times)
...
def _future_steps(m,m_r, x, y, steps=1):
    res = []
    m.eval()
    for i in range(len(x)):
        

        rec = m_r(x[i].unsqueeze(0))
        m.set_xi(rec[-1])
        _= m()
...
def future_steps(m,m_r, x, y ):
    to_return = []

    steps = [1,2,5,8,10,15,20,25,30]

    for s in steps:
        to_return.append({"steps": s, "result": _future_steps(m,m_r, x, y, s)})
...
def evaluate_model(m,m_r,x,y, x_test,y_test,conf, draw_images=True):
    ...
    to_add["future"] = future_steps(m, m_r, x, y)
    to_add["future_eval"] = evaluate_future_steps(to_add["future"])

    to_add["test_run"] = generate_test(m, m_r, conf)
